...
class Game_Surname {
public:
    std::vector<Game_surname_kishiev> players;
    ...
    void applyBattleResults(Game_surname_kishiev& player, int battleResult, std::vector<Adviser>& advisers, int year, std::string enemy);
    void start();
};
...
void Game_Surname::stage1() {
    std::cout << "Welkome to Kingsburg \n stage 1: initiation" << std::endl;
    std::cout << "enter number players: ";
    int numPlayers;
    std::cin >> numPlayers;

    for (int i = 0; i < numPlayers; i++) {
        Game_surname_kishiev player;
...
void Game_Surname::stage4() {
    std::cout << "stage 4: Build Buildings and Roll Dice" << std::endl;
    rollDice();
    changePlayerOrder();
    influenceAdvisers();
    getAdviserHelp();
...
void Game_Surname::stage5() {
    ...
    std::cout << "enemy attack your castle!" << std::endl;
...
void Game_Surname::stage6() {
    std::cout << "stage 6: Build Buildings and Roll Dice" << std::endl;
    rollDice();
    changePlayerOrder();
    influenceAdvisers();
    getAdviserHelp();
...
void Game_Surname::stage8() {
    std::cout << "stage 8: End of Year" << std::endl;

    year++;
    std::cout << "Year " << year << " has ended." << std::endl;
    for (int i = 0; i < players.size(); i++) {
        int scores = players[i].gold + players[i].wood + players[i].tokens + players[i].cube1 + players[i].cube2 + players[i].cube3 + players[i].additionalCube;
        players[i].victoryScore = scores;
    }
    ...
    int highestScore = 0;
    for (int i = 0; i < players.size(); i++) {
        if (players[i].victoryScore > highestScore) {
            gameOver = true;
            winner = players[i].name;
            highestScore = players[i].victoryScore;
        }
    }
